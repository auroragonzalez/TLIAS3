...
def transfer_learning(cluster, lastCommits, resWO, resW, count, n_muestras):
    ...
    json_file = open(dir_base + "/transfer-learning/modelsProtoScaled5/mod" + str(cluster+1) + ".json", 'r')
    ...
    for nm in indx:
        ...
        n_epochs = 300
        model = create_model(N_OUT, n_seq, 24, n_features)                  
        mod, pred = FitForecast_e(model = model, X_train=X, y_train=y, X_test=test_x, N_OUT=24, n_seq= n_seq, n_substeps=24, n_features=n_features, epochs=n_epochs,number=0, trained_model=None)
        ...
        model = create_model(N_OUT, n_seq, 24, n_features)
        mod, pred = FitForecast_e(model, X_train=X, y_train=y, X_test=test_x, N_OUT=24, n_seq=n_seq, n_substeps=24, n_features=n_features, epochs=n_epochs,number=0, trained_model=loaded_model)
        pred_resc = scaler_cons.inverse_transform(pred.reshape(-1, 1))
        mae_, mse_, rmse_, cvrmse_, mape_ = compute_metrics_fn(test_y_resc.reshape(-1), pred_resc.reshape(-1))
        print(cvrmse_)
        ...
        if rnd > 1:
            for epoch in range(epochs):
                ...
                self.model, pred = FitForecast_e(self.model, self.x_train, self.y_train, self.x_test, 24, self.n_seq,
                                                 24, self.n_features, epochs, self.party_number, trained_model=None)
            ...
            self.model, pred = FitForecast_e(self.model, self.x_train, self.y_train, self.x_test, 24, self.n_seq, 24,
                                             self.n_features, epochs, self.party_number, trained_model=None)
