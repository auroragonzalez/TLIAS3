...
def kprot_missing(X, n_clusters, cat_index, max_iter=10):  # cat_index has to be a vector: [5], or [3,4,5] , never an integer
    """Perform K-prot clustering on data with missing values.
    Args:
...
def labels_cost(Xnum, Xcat, centroids, num_dissim, cat_dissim, gamma, labels, membship=None):
...
for(ncl in NCLUSTERS):
    labels, centroids, X_hat, gamma = kprot_missing(df, n_clusters=5, cat_index = [1,2,3,5,7,8,10], max_iter=1000)
    ...
    distances = labels_cost(Xnum, Xcat, centroids, num_dissim, cat_dissim, gamma, labels)
    df2 = pd.DataFrame(df)
    X_hat2 = pd.DataFrame(X_hat)
    labels2dist = pd.DataFrame(labels, distances)
    centroids2 = pd.DataFrame(centroids)
    df2.to_csv("kprototypes-df",+ncl,".csv", sep=';')
    X_hat2.to_csv("kprototypes-X_hat",+ncl,".csv", sep=';')
    labels2dist.to_csv("kprototypes-labels-dist",+ncl,".csv", sep=';')
    centroids2.to_csv("kprototypes-centroids",+ncl,".csv", sep=';')
