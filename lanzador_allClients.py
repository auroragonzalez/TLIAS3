...
parametros.add_argument("cluster", help="cluster", type=int)
...
#labelsAndDist = pd.read_csv('../k-prod/kprototypes-labels-dist-iii15FIJO.csv', sep=",")
labelsAndDist = pd.read_csv(dir_base + '/01.k-prot-clustering/kprototypes-labels-dist-iii15.csv', sep=",")


for i in k:
    indx = labelsAndDist[labelsAndDist[labelsAndDist.columns[1]] == i].index
    print(i)
    for nm in indx:
        lastCommit = refs['HEAD']
